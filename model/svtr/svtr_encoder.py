...
class AttnMixer(BaseModule):
    ...
    def __init__(
        ...
        if mixer == "Local" and input_shape is not None:
            ...
            wk = local_k[1]
            mask = torch.ones(
                [height * width, height + hk - 1, width + wk - 1], dtype=torch.float32
            )
            for h in range(0, height):
                for w in range(0, width):
                    mask[h * width + w, h : h + hk, w : w + wk] = 0.0
            mask = mask[
                :, hk // 2 : height + hk // 2, wk // 2 : width + wk // 2
            ].flatten(1)
            mask[mask >= 1] = -np.inf
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv = (
            self.qkv(x)
            .reshape((-1, input_size, 3, self.num_heads, embed_dims // self.num_heads))
            .permute((2, 0, 3, 1, 4))
        )
...
class SVTREncoder(BaseModule):
    ...
    Args:
        ...
        window_size (Tuple[Tuple[int, int]], optional):
          ...
          (str, optional): The way of downsample in MergingBlock.
            Defaults to 'Conv'.
    ...
    def __init__(
        ...
        self.patch_embed = OverlapPatchEmbed(
            in_channels=in_channels, embed_dims=embed_dims[0], num_layers=num_layers
        )
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.downsample1(
            x.permute(0, 2, 1).reshape(
                [-1, self.embed_dims[0], self.input_shape[0], self.input_shape[1]]
            )
        )
        ...
        x = self.downsample2(
            x.permute(0, 2, 1).reshape(
                [-1, self.embed_dims[1], self.input_shape[0] // 2, self.input_shape[1]]
            )
        )
    ...
    def forward(
        self, x: torch.Tensor, data_samples: List[TextRecogDataSample] = None
    ) -> torch.Tensor:
        """Forward function.
        ...
        x = self.avgpool(
            x.permute(0, 2, 1).reshape(
                [-1, self.embed_dims[2], self.input_shape[0] // 4, self.input_shape[1]]
            )
        )
        x = self.last_conv(x)
        x = self.hardwish(x)
