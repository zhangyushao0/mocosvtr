...
class TPStransform(nn.Module):
    ...
    def __init__(
        ...
        tgt_coord_repr = torch.cat(
            [tgt_coord_partial_repr, torch.ones(HW, 1), tgt_coord], dim=1
        )
    ...
    def forward(
        self, input: torch.Tensor, source_control_points: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _compute_partial_repr(
        self, input_points: torch.Tensor, control_points: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _build_output_control_points(
        self, num_control_points: torch.Tensor, margins: Tuple[float, float]
    ) -> torch.Tensor:
...
class STN(BasePreprocessor):
    ...
    def __init__(
        ...
        self.stn_fc1 = nn.Sequential(
            nn.Linear(
                256
                * (self.resized_image_size[0] // 32)
                * (self.resized_image_size[1] // 32),
                512,
            ),
            nn.BatchNorm1d(512),
